...
최근에는 반복되는 개발 업무와 프로젝트 설정을 내부 도구와 workflow로 구조화하고, AI 결과를 테스트·CI·독립 리뷰·사람 승인으로 검증하는 공개 engineering 프로젝트를 진행하고 있습니다. 백엔드 중심이지만 업무 화면, API, 데이터 흐름을 함께 이해하고 필요한 도구를 직접 설계·구현하는 역할을 지향합니다.
